...
### 2. 画布与视图控制
- **图片作废**：如果某张图片不需要标注，可勾选图片**左上角的“作废该页”**，图片将变暗且不可交互，JSON 数据中会增加 `ignored: true` 标识。
...
### 3. 编辑与重构
- <kbd>T</kbd> **重构为新题**：**按住** `T` 键并连续点击多个已有标注框（题目/答案/批改），这些框会被提取出来，并直接**合并转化为一个全新的独立题目（题干）**。松开 `T` 键即结束本次重构。如果原题组因此变空，则会被自动删除。
